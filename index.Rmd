...
R is free and there are many official packages available (check [CRAN list](http://cran.r-project.org/web/packages/available_packages_by_name.html)).

Also many people create their own packages to distribute their work. Most of them are hosted in [github](http://github.com) or similar websites. In this workshop you will be using a custom package developed in this Department.
...
plot(co2~year,data=data.co2_melt,xlim=c(min(data.co2_melt$year),2020),ylim=c(min(data.co2_melt$co2),max(predictions)))
points(1997:2020,predictions,col='red')
```

---

## So, I am in spectroscopy workshop... but...
