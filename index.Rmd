...
A plot is worth a thousand words... R is capable of plotting data in many different ways. Let's start with a basic function:
...
Another simple and useful plot is the boxplot. Remember that we can fix up those messy axis labels with the xlab and ylab arguments. We'll use another sample dataset (`CO2`) so remember to check its structure. It's a dataset regarding CO<sub>2</sub> uptake in plants. You can find out more by using `?CO2`
...
```{r}
data.co2 <- as.data.frame(matrix(co2, 12, byrow=TRUE), row.names=F)
